...
export default function Accordion({ children, title, id }: AccordionProps) {
    ...
    <div className="py-2">
      <h2>
        <button
          ...
          <svg
            className="fill-white shrink-0 mr-5"
            width="16"
            height="16"
          ...
          <span className={accordionOpen ? 'animated-text' : ''}>{title}</span>
        </button>
      </h2>
      <div
        ...
        <div className="overflow-hidden">
          <p className="accordion-title font-light">{children}</p>
        </div>
      </div>
    </div>
  );
